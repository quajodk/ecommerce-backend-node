  ...
  try {
    ...
    if (existingUser) {
      return res.status(400).json({
        success: false,
        error: "User with email already exist. Login",
      });
    }
...
exports.loginAdmin = async (req, res) => {
  try {
    ...
    const headerToken = isAuth.split(" ")[1];
...
// desc Log user in by id
// @route POST api/v1/auth/me/login
// @access Private
exports.loginById = async (req, res, next) => {
  try {
    ...
    if (headerToken) {
      ...
      const token = helper.token(Array.from(id).length);
      const newToken = await Token.create({
        token,
        login: true,
        user: user._id,
