...
exports.addItems = async (req, res, next) => {
  try {
    ...
    const total =
      itemsArray.length !== 0
        ? itemsArray
            .map(async (value) => await Product.findById(value))
            .reduce((acc, cur) => acc + Number(cur.details.price), 0)
        : 0;
...
exports.removeItem = async (req, res, next) => {
  try {
    ...
    const remainCartItem = cart.items.filter(
...
exports.checkout = async (req, res, next) => {
  try {
    ...
    const checkoutCart = await Cart.updateOne(
      { _id: id },
      {
        $set: {
          checkout: true,
